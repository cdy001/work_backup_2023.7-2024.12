...
int main(int argc, char* argv[])
{
    ...
    // 打印 dies[0] 的内容
    const Die& die = dies[0];
    for (const Die die: dies){
        // cout << die.x_min << " " << die.y_min << " " << die.x_max << " " << die.y_max << endl;
        cv::Point pt1(die.x_min - int(args["margin_x"]), die.y_min - int(args["margin_y"]));
        cv::Point pt2(die.x_max + int(args["margin_x"]), die.y_max + int(args["margin_y"]));
